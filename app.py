...
import sys, os, logging, re, hashlib, time, threading, copy, random, json
...
# ANOMALY DETECTION
# ══════════════════════════════════════════════════════════
def detect_anomalies(telemetry: List[dict], logs: List[dict] = None) -> List[dict]:
    """
    Full anomaly detection — telemetry thresholds + log pattern analysis.
    Covers: CPU, memory, packet loss, interface errors, BGP, OSPF,
            IP SLA (RTT/jitter/MOS/loss/timeout), MPLS, STP, security events.
    """
    anomalies = []
    logs = logs or []
    ...
    for t in telemetry:
        ...
        if t.get("cpu", 0) >= 85:
            anomalies.append({
                "device": hn, "type": "cpu_high", "severity": "critical",
                "value": f"{t['cpu']}%",
                "message": f"{hn} CPU {t['cpu']}% — possible route churn or DoS",
                "ai_hint": "Run: show processes cpu sorted. Common causes: OSPF SPF, BGP scanner, ACL hits",
                "category": "Resource",
            })
        elif t.get("cpu", 0) >= 70:
            anomalies.append({
                "device": hn, "type": "cpu_elevated", "severity": "warning",
                "value": f"{t['cpu']}%",
                "message": f"{hn} CPU elevated at {t['cpu']}%",
                "ai_hint": "Monitor for escalation. Show processes cpu sorted.",
                "category": "Resource",
            })

        # Packet loss
        loss = t.get("packet_loss", 0)
        if loss > 0.5:
            anomalies.append({
                "device": hn, "type": "packet_loss",
                "severity": "critical" if loss > 1.5 else "warning",
                "value": f"{loss}%",
                "message": f"{hn} packet loss {loss}% — SLA breach likely",
                "ai_hint": "Check interface error counters, QoS tail-drop, hardware queue, duplex mismatch",
        ...
        for intf in t.get("interfaces", []):
            if intf.get("errors", 0) > 2:
                anomalies.append({
                    "device": hn, "type": "interface_errors", "severity": "warning",
                    "value": str(intf["errors"]),
                    "message": f"{hn} {intf['interface']} CRC/input errors: {intf['errors']}",
                    "ai_hint": "Check cable quality, SFP, duplex mismatch: show interfaces | inc error",
                    "category": "Interface",
                })
        ...
        for bgp in t.get("bgp_sessions", []):
            if bgp.get("state") == "Active":
                anomalies.append({
                    "device": hn, "type": "bgp_active", "severity": "critical",
                    "value": bgp["peer"],
                    "message": f"{hn} BGP peer {bgp['peer']} stuck in Active (TCP not established)",
                    "ai_hint": "Check: ping peer, TCP 179 reachability, remote-as, MD5 auth, update-source",
                    "category": "BGP",
                })

    # ── Log-based detection — IP SLA + all types ──────────
    IPSLA_RTT_THRESHOLD_MS    = 150
    IPSLA_JITTER_THRESHOLD_MS = 30
    IPSLA_LOSS_THRESHOLD_PCT  = 5
    IPSLA_MOS_THRESHOLD       = 3.5
    ...
    for log in logs:
        ...
        if not log.get("needs_action") and sev not in ("critical","error","warning"):
            continue

        # IP SLA — detailed threshold check
        if ltype in ("ipsla", "ipsla_probe_result", "ipsla_track"):
            probe_anomalies = []

            rtt = ext.get("rtt")
            if rtt and int(rtt) > IPSLA_RTT_THRESHOLD_MS:
                probe_anomalies.append(f"RTT {rtt}ms > {IPSLA_RTT_THRESHOLD_MS}ms threshold")

            jitter = ext.get("jitter")
            if jitter and int(jitter) > IPSLA_JITTER_THRESHOLD_MS:
                probe_anomalies.append(f"Jitter {jitter}ms > {IPSLA_JITTER_THRESHOLD_MS}ms threshold")

            loss = ext.get("loss")
            if loss and int(loss) > IPSLA_LOSS_THRESHOLD_PCT:
                probe_anomalies.append(f"Loss {loss}% > {IPSLA_LOSS_THRESHOLD_PCT}% threshold")

            mos = ext.get("mos")
            if mos and float(mos) < IPSLA_MOS_THRESHOLD:
                probe_anomalies.append(f"MOS {mos} < {IPSLA_MOS_THRESHOLD} (voice degraded)")

            state = ext.get("state", "")
            if state.lower() == "down":
                probe_anomalies.append("Tracked object DOWN — failover may trigger")

            if probe_anomalies or sev == "critical":
                probe_id = ext.get("probe_id", "?")
                anomalies.append({
                    "device":   dev,
                    "type":     "ipsla_violation",
                    "severity": sev,
                    "value":    " | ".join(probe_anomalies) if probe_anomalies else "probe timeout",
                    "message":  f"{dev} IP SLA probe {probe_id}: {', '.join(probe_anomalies) or 'timeout/failure'}",
                    "ai_hint":  log.get("ai_hint", _ipsla_hint(ext)),
                    "category": "IP SLA",
                    "raw_log":  log.get("raw", ""),
                })

        # BGP log-driven
        elif ltype in ("bgp_adjchange", "bgp_notification"):
            if ext.get("state", "").lower() in ("down", "reset") or ltype == "bgp_notification":
                anomalies.append({
                    "device":   dev, "type": "bgp_down", "severity": "critical",
                    "value":    ext.get("peer", ""),
                    "message":  log.get("message", "BGP session down"),
                    "ai_hint":  log.get("ai_hint", "Check BGP peer connectivity"),
                    "category": "BGP", "raw_log": log.get("raw", ""),
                })

        # OSPF log-driven
        elif ltype == "ospf_neighbor" and sev == "critical":
            anomalies.append({
                "device":   dev, "type": "ospf_adjchange", "severity": "critical",
                "value":    ext.get("neighbor", ""),
                "message":  log.get("message", "OSPF adjacency change"),
                "ai_hint":  log.get("ai_hint", "Check OSPF timers and MTU"),
                "category": "OSPF", "raw_log": log.get("raw", ""),
            })

        # Interface down from log
        elif ltype in ("link_updown", "lineproto_updown") and ext.get("state","").lower() == "down":
            anomalies.append({
                "device":   dev, "type": "interface_down", "severity": "critical",
                "value":    ext.get("interface", ""),
                "message":  f"{dev} {ext.get('interface','')} interface/protocol DOWN",
                "ai_hint":  log.get("ai_hint", "Check cable, SFP, remote device"),
                "category": "Interface", "raw_log": log.get("raw", ""),
            })

        # MPLS / LDP
        elif ltype in ("ldp_neighbor", "mpls_te") and sev == "critical":
            anomalies.append({
                "device":   dev, "type": "mpls_fault", "severity": "critical",
                "value":    ext.get("peer", ext.get("tunnel", "")),
                "message":  log.get("message", "MPLS fault"),
                "ai_hint":  log.get("ai_hint", "Check IGP reachability and RSVP signaling"),
                "category": "MPLS", "raw_log": log.get("raw", ""),
            })

        # Security events
        elif ltype in ("aaa_failure", "priv_escalation") or (ltype == "acl_hit" and ext.get("action","").lower() == "denied"):
            anomalies.append({
                "device":   dev, "type": "security_event",
                "severity": "critical" if ltype == "aaa_failure" else "warning",
                "value":    ext.get("user", ext.get("src", "")),
                "message":  log.get("message", "Security event"),
                "ai_hint":  log.get("ai_hint", "Review audit trail and access policies"),
                "category": "Security", "raw_log": log.get("raw", ""),
            })

        # STP topology change
        elif ltype == "stp_event" and ext.get("mnemonic","").upper() in ("TOPOLOGYCHANGE","LOOPGUARD_BLOCK","ROOTGUARD_BLOCK"):
            anomalies.append({
                "device":   dev, "type": "stp_topo_change", "severity": "warning",
                "value":    f"VLAN {ext.get('vlan','')}",
                "message":  f"{dev} STP topology change on VLAN {ext.get('vlan','')}",
                "ai_hint":  log.get("ai_hint", "Check for rogue switch, PortFast misconfiguration"),
                "category": "Spanning Tree", "raw_log": log.get("raw", ""),
            })

    # Deduplicate by (device, type, value) and sort by severity
    seen, deduped = set(), []
    for a in anomalies:
        key = (a["device"], a["type"], a.get("value",""))
        if key not in seen:
            seen.add(key)
            deduped.append(a)

    return sorted(deduped, key=lambda x: {"critical":0,"error":1,"warning":2,"info":3}.get(x["severity"],4))
...
# ══════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════
# SYSLOG PARSER — ALL LOG TYPES (IP SLA, BGP, OSPF, Interface, Security, etc.)
# ══════════════════════════════════════════════════════════
...
SYSLOG_SEVERITY_MAP = {
    ...
    "6": "info",      "7": "debug",
    # Cisco mnemonic → normalized
    "ADJCHANGE":  "critical", "NOTIFICATION": "critical",
    "UPDOWN":     "critical", "CHANGED":       "warning",
    "THRESHOLD":  "warning",  "VIOLATION":     "critical",
    "TIMEOUT":    "critical", "RTT":           "warning",
    "JITTER":     "warning",  "MOS":           "critical",
    "PACKETLOSS": "critical", "VERIFY_ERROR":  "critical",
    "STATE_CHANGE":"warning", "NONEIGHBOR":    "warning",
    "REACHABILITY":"critical","PROBELOSS":     "critical",
    "RESPONSE_TIME":"warning","IPSLA":         "info",
    "ICMP_ECHO":  "info",     "UDP_JITTER":    "info",
}
...
SYSLOG_PATTERNS = [
    ...
    {
        "pattern": re.compile(
            r'%IPSLA-\d-(\w+).*?IP SLA.*?(\d+).*?'
            r'(?:RTT=(\d+)|jitter=(\d+)|MOS=(\d+\.\d+)|loss=(\d+)%?|threshold.*?violation)?',
            re.IGNORECASE
        ),
        "type": "ipsla",
        "category": "IP SLA",
        "extract": lambda m: {
            "mnemonic": m.group(1) or "",
            "probe_id": m.group(2) or "",
            "rtt":      m.group(3),
            "jitter":   m.group(4),
            "mos":      m.group(5),
            "loss":     m.group(6),
        },
        "severity_fn": lambda m: "critical" if any(w in (m.group(1) or "").upper()
            for w in ["VIOLATION","TIMEOUT","PROBELOSS","REACHABILITY","THRESHOLD"]) else "warning",
        "ai_hint_fn": lambda d: (
            "IP SLA probe timeout — check end-to-end path, CE router reachability, return path routing" if "TIMEOUT" in d.get("mnemonic","").upper()
            else "MOS degraded — voice quality impacted, check jitter/loss on UDP jitter probe path" if "MOS" in d.get("mnemonic","").upper()
            else "RTT threshold crossed — latency SLA breach, check WAN link utilization and QoS" if "RTT" in d.get("mnemonic","").upper()
            else "IP SLA threshold violation — review probe targets, check path and QoS policy"
        ),
    },
    {
        "pattern": re.compile(
            r'%TRACK-\d-STATE.*?(\d+).*?ip sla.*?reachability.*?'
            r'(Up|Down|Unknown)', re.IGNORECASE
        ),
        "type": "ipsla_track",
        "category": "IP SLA Tracking",
        "extract": lambda m: {"track_id": m.group(1), "state": m.group(2)},
        "severity_fn": lambda m: "critical" if (m.group(2) or "").lower() == "down" else "info",
        "ai_hint_fn": lambda d: (
            "Tracked object DOWN — failover may trigger. Check IP SLA probe target and return routing"
            if d.get("state","").lower() == "down"
            else "Tracked object recovered UP — failover reverted or standby path restored"
        ),
    },
    {
        "pattern": re.compile(
            r'IP SLA\s+(\d+).*?'
            r'(?:(RTT|rtt)[=:\s]+(\d+)\s*(?:ms|msec))?.*?'
            r'(?:(Jitter|jitter)[=:\s]+(\d+)\s*(?:ms|msec))?.*?'
            r'(?:(Loss|PacketLoss)[=:\s]+(\d+)%?)?.*?'
            r'(?:(MOS|mos)[=:\s]+(\d+\.?\d*))?',
            re.IGNORECASE
        ),
        "type": "ipsla_probe_result",
        "category": "IP SLA Probe Result",
        "extract": lambda m: {
            "probe_id": m.group(1),
            "rtt":      m.group(3),
            "jitter":   m.group(5),
        ...
        "severity_fn": lambda m: (
            "critical" if (m.group(7) and int(m.group(7)) > 5)
            else "warning" if (m.group(3) and int(m.group(3)) > 150)
            else "info"
        ),
        "ai_hint_fn": lambda d: _ipsla_hint(d),
    },
    # ── BGP ────────────────────────────────────────────────
    {
        "pattern": re.compile(r'%BGP-\d-ADJCHANGE.*?neighbor\s+([\d.]+).*?(Up|Down|reset|established)', re.IGNORECASE),
        "type": "bgp_adjchange",
        "category": "BGP",
        "extract": lambda m: {"peer": m.group(1), "state": m.group(2)},
        "severity_fn": lambda m: "critical" if m.group(2).lower() in ("down","reset") else "info",
        "ai_hint_fn": lambda d: (
            f"BGP peer {d.get('peer')} went down — check TCP 179 reachability, hold-timer, MD5 auth, and ISP status"
            if d.get("state","").lower() in ("down","reset")
            else f"BGP peer {d.get('peer')} established — session recovered"
        ),
    },
    {
        "pattern": re.compile(r'%BGP-\d-NOTIFICATION.*?neighbor\s+([\d.]+).*?(?:Hold Timer|hold timer|error code\s+(\d+))', re.IGNORECASE),
        "type": "bgp_notification",
        "category": "BGP",
        "extract": lambda m: {"peer": m.group(1), "error": m.group(2) or "hold-timer"},
        "severity_fn": lambda m: "critical",
        "ai_hint_fn": lambda d: f"BGP NOTIFICATION from {d.get('peer')} — hold-timer expired. Increase hold-timer or enable BFD. Check network stability.",
    },
    {
        "pattern": re.compile(r'%BGP-\d-(\w+).*?prefix.*?([\d.]+/\d+).*?'
                               r'(?:exceeded|limit|withdraw|flap)', re.IGNORECASE),
        "type": "bgp_prefix",
        "category": "BGP",
        "extract": lambda m: {"mnemonic": m.group(1), "prefix": m.group(2)},
        "severity_fn": lambda m: "warning",
        "ai_hint_fn": lambda d: f"BGP prefix issue for {d.get('prefix')} — check route policy, prefix limits, and ISP advertisements",
    },
    # ── OSPF ───────────────────────────────────────────────
    {
        "pattern": re.compile(r'%OSPF-\d-(\w+).*?(?:neighbor|Nbr)\s+([\d.]+).*?'
                               r'(?:interface|from\s+(\w+)\s+to\s+(\w+))?', re.IGNORECASE),
        "type": "ospf_neighbor",
        "category": "OSPF",
        "extract": lambda m: {"mnemonic": m.group(1), "neighbor": m.group(2),
                              "from_state": m.group(3), "to_state": m.group(4)},
        "severity_fn": lambda m: "critical" if m.group(1).upper() in ("ADJCHG","NOFULLNBRS","BADLSATYPE") else "warning",
        "ai_hint_fn": lambda d: (
            f"OSPF adjacency change with {d.get('neighbor')} from {d.get('from_state')} to {d.get('to_state')} — "
            "check MTU mismatch (ip ospf mtu-ignore), hello/dead timers, auth"
        ),
    },
    # ── Interface / Link ───────────────────────────────────
    {
        "pattern": re.compile(r'%LINK-\d-UPDOWN.*?Interface\s+([\w./]+).*?changed state to\s+(\w+)', re.IGNORECASE),
        "type": "link_updown",
        "category": "Interface",
        "extract": lambda m: {"interface": m.group(1), "state": m.group(2)},
        "severity_fn": lambda m: "critical" if m.group(2).lower() == "down" else "info",
        "ai_hint_fn": lambda d: (
            f"Interface {d.get('interface')} went DOWN — check cable/SFP, remote device, duplex mismatch, err-disable reason"
            if d.get("state","").lower() == "down"
            else f"Interface {d.get('interface')} came UP — verify services restored"
        ),
    },
    {
        "pattern": re.compile(r'%LINEPROTO-\d-UPDOWN.*?Interface\s+([\w./]+).*?changed state to\s+(\w+)', re.IGNORECASE),
        "type": "lineproto_updown",
        "category": "Interface",
        "extract": lambda m: {"interface": m.group(1), "state": m.group(2)},
        "severity_fn": lambda m: "critical" if m.group(2).lower() == "down" else "info",
        "ai_hint_fn": lambda d: (
            f"Line protocol DOWN on {d.get('interface')} — L2 issue, check encapsulation, keepalives, remote end"
            if d.get("state","").lower() == "down"
            else f"Line protocol UP on {d.get('interface')} — L2 session restored"
        ),
    },
    {
        "pattern": re.compile(r'%ETHPORT-\d-(\w+).*?interface\s+([\w./]+)', re.IGNORECASE),
        "type": "eth_port",
        "category": "Interface",
        "extract": lambda m: {"mnemonic": m.group(1), "interface": m.group(2)},
        "severity_fn": lambda m: "warning",
        "ai_hint_fn": lambda d: f"Ethernet port event {d.get('mnemonic')} on {d.get('interface')} — check err-disable, duplex, speed",
    },
    # ── CPU / Memory ───────────────────────────────────────
    {
        "pattern": re.compile(r'%CPU_MONITOR.*?THRESHOLD.*?CPU\s+Utilization\((\d+)%\)', re.IGNORECASE),
        "type": "cpu_threshold",
        "category": "Resource",
        "extract": lambda m: {"cpu_pct": m.group(1)},
        "severity_fn": lambda m: "critical" if int(m.group(1)) >= 85 else "warning",
        "ai_hint_fn": lambda d: f"CPU {d.get('cpu_pct')}% — show processes cpu sorted. Common causes: OSPF SPF, BGP scanner, ACL hits, routing loops",
    },
    {
        "pattern": re.compile(r'%SYS-\d-MEMERR.*?(?:allocated|available|low).*?(\d+)\s*(?:bytes|KB|MB)?', re.IGNORECASE),
        "type": "memory_error",
        "category": "Resource",
        "extract": lambda m: {"amount": m.group(1)},
        "severity_fn": lambda m: "critical",
        "ai_hint_fn": lambda d: "Memory error — reload may be required. Check memory leaks with 'show memory statistics'. Consider upgrade.",
    },
    # ── Security ───────────────────────────────────────────
    {
        "pattern": re.compile(
            r'%(?:SEC|ACL|IPACCESSLOG\w*).*?(?:denied|blocked|permit)\s+(\w+)\s+'
            r'([\d.]+)\((\d+)\)\s*-+>\s*([\d.]+)\((\d+)\)',
            re.IGNORECASE
        ),
        "type": "acl_hit",
        "category": "Security",
        "extract": lambda m: {"action": m.group(1), "src": m.group(2), "sport": m.group(3),
                              "dst": m.group(4), "dport": m.group(5)},
        "severity_fn": lambda m: "warning" if m.group(1).lower() == "denied" else "info",
        "ai_hint_fn": lambda d: (
            f"ACL denied {d.get('src')}:{d.get('sport')} → {d.get('dst')}:{d.get('dport')} — "
            "check if legitimate traffic blocked, review ACL order and implicit deny"
        ),
    },
    {
        "pattern": re.compile(r'%AAA.*?(?:FAIL|failed|rejected).*?user\s+(\S+)', re.IGNORECASE),
        "type": "aaa_failure",
        "category": "Security",
        "extract": lambda m: {"user": m.group(1)},
        "severity_fn": lambda m: "critical",
        "ai_hint_fn": lambda d: f"AAA auth failure for user {d.get('user')} — check TACACS/RADIUS reachability, credentials, lockout policy",
    },
    {
        "pattern": re.compile(r'%SYS-\d-PRIV_AUTH_PASS.*?user\s+(\S+).*?privilege\s+level\s+(\d+)', re.IGNORECASE),
        "type": "priv_escalation",
        "category": "Security",
        "extract": lambda m: {"user": m.group(1), "level": m.group(2)},
        "severity_fn": lambda m: "warning" if int(m.group(2)) >= 15 else "info",
        "ai_hint_fn": lambda d: f"User {d.get('user')} elevated to privilege {d.get('level')} — verify authorized change, review audit trail",
    },
    # ── MPLS / VPN ─────────────────────────────────────────
    {
        "pattern": re.compile(r'%MPLS_TE-\d-(\w+).*?(?:tunnel|LSP).*?([\w.-]+)', re.IGNORECASE),
        "type": "mpls_te",
        "category": "MPLS",
        "extract": lambda m: {"mnemonic": m.group(1), "tunnel": m.group(2)},
        "severity_fn": lambda m: "critical" if "DOWN" in m.group(1).upper() or "FAIL" in m.group(1).upper() else "warning",
        "ai_hint_fn": lambda d: f"MPLS TE {d.get('mnemonic')} on {d.get('tunnel')} — check RSVP signaling, path constraints, CSPF calculation",
    },
    {
        "pattern": re.compile(r'%LDP-\d-(\w+).*?(?:neighbor|peer)\s+([\d.]+)', re.IGNORECASE),
        "type": "ldp_neighbor",
        "category": "MPLS",
        "extract": lambda m: {"mnemonic": m.group(1), "peer": m.group(2)},
        "severity_fn": lambda m: "critical" if "DOWN" in m.group(1).upper() else "warning",
        "ai_hint_fn": lambda d: f"LDP {d.get('mnemonic')} with peer {d.get('peer')} — MPLS label distribution affected, check IGP reachability to LDP peer",
    },
    # ── Spanning Tree ──────────────────────────────────────
    {
        "pattern": re.compile(r'%SPANTREE-\d-(\w+).*?(?:VLAN|vlan)\s*(\d+).*?'
                               r'(?:port\s+([\w./]+))?', re.IGNORECASE),
        "type": "stp_event",
        "category": "Spanning Tree",
        "extract": lambda m: {"mnemonic": m.group(1), "vlan": m.group(2), "port": m.group(3)},
        "severity_fn": lambda m: "critical" if m.group(1).upper() in ("LOOPGUARD_BLOCK","ROOTGUARD_BLOCK","TOPOLOGYCHANGE") else "warning",
        "ai_hint_fn": lambda d: (
            f"STP topology change on VLAN {d.get('vlan')} — MAC table flush triggered. Check {d.get('port','port')} for BPDU source, portfast misconfiguration"
        ),
    },
    # ── SD-WAN / OMP ───────────────────────────────────────
    {
        "pattern": re.compile(r'%SDWAN-\d-(\w+).*?(?:OMP|BFD|TLOc).*?([\d.]+)', re.IGNORECASE),
        "type": "sdwan_event",
        "category": "SD-WAN",
        "extract": lambda m: {"mnemonic": m.group(1), "peer": m.group(2)},
        "severity_fn": lambda m: "critical" if any(w in m.group(1).upper() for w in ["DOWN","FAIL","LOSS"]) else "warning",
        "ai_hint_fn": lambda d: f"SD-WAN {d.get('mnemonic')} — check OMP/BFD session with {d.get('peer')}, TLOC reachability, vSmart controller connectivity",
    },
    # ── Fallback — any unmatched ────────────────────────────
    {
        "pattern": re.compile(r'%(\w+)-(\d)-(\w+):(.*)', re.IGNORECASE),
        "type": "generic",
        "category": "General",
        "extract": lambda m: {"facility": m.group(1), "level": m.group(2),
                              "mnemonic": m.group(3), "text": m.group(4)},
        "severity_fn": lambda m: (
            "critical" if int(m.group(2)) <= 3
            else "warning" if int(m.group(2)) <= 4
            else "info"
        ),
        "ai_hint_fn": lambda d: f"{d.get('facility')}-{d.get('mnemonic')}: review Cisco documentation for mnemonic {d.get('mnemonic')}",
    },
]


def _ipsla_hint(d: dict) -> str:
    """Generate AI hint for IP SLA probe results."""
    hints = []
    if d.get("loss") and int(d.get("loss", 0)) > 5:
        hints.append(f"packet loss {d['loss']}% — check QoS drop, congestion, hardware queues")
    if d.get("rtt") and int(d.get("rtt", 0)) > 150:
        hints.append(f"RTT {d['rtt']}ms exceeds SLA — check WAN utilization, QoS policy, routing path")
    if d.get("jitter") and int(d.get("jitter", 0)) > 30:
        hints.append(f"jitter {d['jitter']}ms — voice quality impacted, check WAN buffer, QoS DSCP EF marking")
    if d.get("mos") and float(d.get("mos", 5.0)) < 3.5:
        hints.append(f"MOS {d['mos']} below 3.5 — unacceptable voice quality, check loss and jitter simultaneously")
    return ". ".join(hints) if hints else "IP SLA probe result — review thresholds against SLA requirements"


def parse_log_line(raw_line: str, device: str = "unknown") -> dict:
    """
    Parse a single syslog line — matches ALL pattern types including IP SLA.
    Returns a structured log dict ready for AI analysis and UI display.
    """
    raw_line = raw_line.strip()
    if not raw_line:
        return {}

    # Extract timestamp if present (Cisco format: *MMM DD HH:MM:SS.mmm or NTP timestamp)
    ts_match = re.match(
        r'[*.]?(?:(\w{3}\s+\d+\s+[\d:]+(?:\.\d+)?)|(\d{4}-\d{2}-\d{2}T[\d:.]+Z?))\s*:?\s*',
        raw_line
    )
    ts_str = ""
    body = raw_line
    if ts_match:
        ...
        body = raw_line[ts_match.end():].strip()

    # Try each pattern
    for pat_def in SYSLOG_PATTERNS:
        m = pat_def["pattern"].search(body)
        if m:
            try:
                extracted = pat_def["extract"](m)
                severity  = pat_def["severity_fn"](m)
                ai_hint   = pat_def["ai_hint_fn"](extracted)
            except Exception:
                extracted = {}
                severity  = "info"
                ai_hint   = "Review log for details"

            return {
                "raw":       raw_line,
                "body":      body,
                "ts":        ts_str or datetime.utcnow().strftime("%H:%M:%S"),
                "device":    device,
                "type":      pat_def["type"],
                ...
                "extracted": extracted,
                "ai_hint":   ai_hint,
                "needs_action": severity in ("critical", "error", "emergency", "alert"),
            }

    # No pattern matched — return raw
    return {
        "raw":       raw_line,
        "body":      body,
        "ts":        ts_str or datetime.utcnow().strftime("%H:%M:%S"),
        ...
        "message":   body[:120],
        "extracted": {},
        "ai_hint":   "Unrecognized log format — check facility and mnemonic manually",
        "needs_action": False,
    }


def parse_log_bulk(raw_text: str, default_device: str = "router") -> List[dict]:
    """
    Parse a multi-line log dump (paste or file upload).
    Handles all Cisco/Juniper syslog formats including IP SLA.
    Returns list of parsed log dicts, sorted newest-first.
    """
    logs = []
    ...
    for line in raw_text.splitlines():
        ...
        # Detect hostname lines (e.g. from 'logging host' output: "CORE-RTR-01#")
        host_match = re.match(r'^([A-Z][A-Z0-9_-]{2,})[>#$]\s*$', line)
        if host_match:
            current_device = host_match.group(1)
            continue

        parsed = parse_log_line(line, current_device)
        if parsed:
            logs.append(parsed)
...
def get_recent_syslogs(limit: int = 20) -> List[dict]:
    """
    Return recent syslog events including ALL types: IP SLA, BGP, OSPF,
    Interface, Security, MPLS, STP, SD-WAN, CPU, Memory.
    Checks session_state for uploaded logs first; falls back to simulation.
    """
    try:
        import streamlit as _st
        # Return uploaded/ingested logs if present
        uploaded = _st.session_state.get("ingested_logs", [])
        if uploaded:
    ...
    # Comprehensive simulated log set — ALL categories represented
    raw_templates = [
        ("PE-MUM-01",
         "%BGP-5-ADJCHANGE: neighbor 10.0.1.1 VRF default Down BGP Notification sent error 4 hold time expired"),
        ("CORE-RTR-01",
         "%IPSLA-3-THRESHOLD_VIOLATION: IP SLA 10 RTT=245ms exceeds configured threshold 150ms to destination 10.0.1.1"),
        ("CORE-RTR-01",
         "%IPSLA-5-UDP_JITTER: IP SLA 20 UDP Jitter probe result: RTT=38ms Jitter=28ms PacketLoss=2% MOS=3.2 to 10.0.2.1"),
        ("PE-DEL-01",
         "%IPSLA-3-TIMEOUT: IP SLA 30 ICMP Echo probe to 203.0.113.1 timed out, consecutive failures=3"),
        ("SW-ACC-14",
         "%LINK-3-UPDOWN: Interface GigabitEthernet0/3, changed state to down"),
        ("CORE-RTR-01",
         "%OSPF-5-ADJCHG: Process 1, Nbr 192.168.1.3 on GigabitEthernet0/2 from LOADING to FULL, Loading Done"),
        ("CORE-RTR-01",
         "%CPU_MONITOR-5-RISING_THRESHOLD: CPU Utilization(88%) above threshold(80%)"),
        ("FW-EDGE-01",
         "%SEC-6-IPACCESSLOGP: list OUTSIDE_IN denied tcp 203.0.113.45(58234) -> 10.0.0.1(23), 1 packet"),
        ("PE-DEL-01",
         "%BGP-3-NOTIFICATION: received from neighbor 10.0.2.1 code 4 (Hold Timer Expired) 0 bytes"),
        ("CORE-RTR-01",
         "IP SLA 10 RTT=189ms Jitter=12ms Loss=0% MOS=4.1 threshold RTT exceeded"),
        ("DIST-SW-W",
         "%SPANTREE-5-TOPOLOGYCHANGE: port GigabitEthernet0/1 in vlan 100 is now designated"),
        ("CORE-RTR-01",
         "%LDP-5-NBRCHG: LDP Neighbor 10.0.1.1:0 (1) is DOWN. Reason: Session Hold Timer expired"),
        ("DIST-SW-W",
         "%LINEPROTO-5-UPDOWN: Line protocol on Interface Port-channel1, changed state to up"),
        ("FW-EDGE-01",
         "%AAA-3-BADSERVERPORT: AAA FAIL for user admin from 192.168.1.100: TACACS+ server timeout"),
        ("WLC-HQ-01",
         "%DOT11-6-DISASSOC: Station 00:1A:2B:3C:4D:5E Disassociated from wlan-corp"),
        ("PE-MUM-01",
         "%MPLS_TE-3-TUNNEL_DOWN: Tunnel100 is down, rerouting failed, no alternate path available"),
        ("CORE-RTR-01",
         "%IPSLA-4-RTT_VIOLATION: IP SLA 15 response time 320ms exceeds SLA limit 200ms destination 10.0.5.1"),
        ("PE-MUM-01",
         "%TRACK-6-STATE: 1 ip sla 10 reachability Up -> Down"),
        ("DIST-SW-C",
         "%SW_MATM-4-MACFLAP_NOTIF: Host 00:aa:bb:cc:dd:ee in vlan 100 is flapping between port Gi0/1 and port Gi0/2"),
    ]

    logs = []
    for i, (device, raw_msg) in enumerate(raw_templates[:limit]):
        parsed = parse_log_line(raw_msg, device)
        if parsed:
            # Spread timestamps naturally
            parsed["ts"] = (now - timedelta(minutes=i * rng.randint(1, 6))).strftime("%H:%M:%S")
            logs.append(parsed)
...
# ══════════════════════════════════════════════════════════
    ...
    live_logs = get_recent_syslogs(30)
    anomalies = detect_anomalies(telemetry, live_logs)
    ...
    with tab_syslog:
        # ── Log ingestion ──────────────────────────────────
        st.markdown("**📥 Log Ingestion — Upload or Paste Router Logs**")
        st.caption("Supports all Cisco/Juniper log types: IP SLA, BGP, OSPF, Interface, MPLS, STP, Security, SD-WAN, CPU, Memory")

        ingest_cols = st.columns([0.55, 0.45])
        with ingest_cols[0]:
            pasted_logs = st.text_area(
                "Paste logs here",
                placeholder=(
                    "Paste router logs — any format:\n"
                    "*May 10 14:02:31: %IPSLA-3-THRESHOLD_VIOLATION: IP SLA 10 RTT=245ms...\n"
                    "*May 10 14:03:12: %BGP-5-ADJCHANGE: neighbor 10.0.1.1 Down...\n"
                    "IP SLA 20 RTT=180ms Jitter=35ms Loss=3% MOS=3.1\n"
                    "%LINK-3-UPDOWN: Interface Gi0/3, changed state to down"
                ),
                height=130, key="syslog_paste",
            )
        with ingest_cols[1]:
            uploaded_file = st.file_uploader(
                "Or upload log file (.txt / .log)",
                type=["txt","log","syslog"],
                key="syslog_upload",
            )
            default_device = st.text_input(
                "Default device name (if not in logs)",
                value="Router", key="syslog_device",
            )
            parse_btn = st.button("🔍 Parse & Analyze Logs", type="primary",
                                  use_container_width=True, key="syslog_parse")
        ...
        if parse_btn:
            ...
            if uploaded_file:
                ...
                raw_text = pasted_logs.strip()

            if raw_text:
                parsed = parse_log_bulk(raw_text, default_device)
                st.session_state["ingested_logs"] = parsed
                st.success(f"✅ Parsed **{len(parsed)} log entries** — {sum(1 for l in parsed if l.get('needs_action'))} require action")
            else:
                st.warning("Paste some logs or upload a file first.")

        # ── Display logs ───────────────────────────────────
        logs_to_show = get_recent_syslogs(30)
        using_uploaded = bool(st.session_state.get("ingested_logs"))

        # Category filter
        all_categories = sorted(set(l.get("category","General") for l in logs_to_show))
        filter_cols = st.columns([0.5, 0.25, 0.25])
        with filter_cols[0]:
            cat_filter = st.multiselect(
                "Filter by category", all_categories,
                default=all_categories, key="log_cat_filter",
            )
        with filter_cols[1]:
            sev_filter = st.multiselect(
                "Severity", ["critical","warning","info"],
                default=["critical","warning","info"], key="log_sev_filter",
            )
        with filter_cols[2]:
            action_only = st.checkbox("Needs action only", key="log_action_only")

        filtered = [
            l for l in logs_to_show
            if l.get("category","General") in cat_filter
            and l.get("severity","info") in sev_filter
            and (not action_only or l.get("needs_action"))
        ]

        # Summary strip
        crit_count = sum(1 for l in filtered if l.get("severity") == "critical")
        warn_count = sum(1 for l in filtered if l.get("severity") == "warning")
        ipsla_count= sum(1 for l in filtered if "ipsla" in l.get("type","").lower() or l.get("category")=="IP SLA")
        label = "📋 Uploaded Logs" if using_uploaded else "📋 Simulated Logs (paste your own above)"
        st.markdown(
            f'<div style="display:flex;gap:10px;align-items:center;margin:8px 0;padding:8px 12px;'
            f'background:var(--bg-elevated);border-radius:8px;border:1px solid var(--border-default)">'
            f'<span style="font-size:12px;color:var(--text-secondary)">{label}</span>'
            f'<span class="nb-chip chip-err">🔴 {crit_count} critical</span>'
            f'<span class="nb-chip chip-warn">🟡 {warn_count} warning</span>'
            f'<span class="nb-chip chip-info">📡 {ipsla_count} IP SLA</span>'
            f'<span style="font-size:11px;color:var(--text-tertiary);margin-left:auto">{len(filtered)} shown</span>'
            f'</div>',
            unsafe_allow_html=True,
        )

        # AI action on all critical logs
        action_logs = [l for l in filtered if l.get("needs_action")]
        if action_logs:
            ai_col1, ai_col2 = st.columns([0.65, 0.35])
            with ai_col1:
                st.markdown(
                    f'<div class="nb-ai-insight"><div class="nb-ai-hdr">🧠 AI Action Required — {len(action_logs)} critical events</div>'
                    f'<div class="nb-ai-body">These logs indicate active faults. AI can correlate root causes, '
                    f'cross-reference with topology, and generate remediation steps.</div></div>',
                    unsafe_allow_html=True,
                )
            with ai_col2:
                if st.button("🧠 AI Root Cause All Logs", type="primary", use_container_width=True, key="syslog_ai_all"):
                    log_summary = "\n".join(
                        f"[{l.get('device')}] [{l.get('category')}] {l.get('message','')}"
                        for l in action_logs[:15]
                    )
                    go(
                        f"Analyze these network logs and identify root causes:\n\n{log_summary}\n\n"
                        "Provide: 1) Root cause correlation 2) Which logs are symptoms vs root cause "
                        "3) Business impact 4) Prioritized remediation steps 5) IP SLA violations and what they indicate",
                        "troubleshoot",
                        f"Log analysis — {len(action_logs)} critical events across {len(set(l['device'] for l in action_logs))} devices",
                    )

        # Log list
        category_icons = {
            "IP SLA": "📡", "BGP": "🔄", "OSPF": "🔁", "Interface": "🔌",
            "MPLS": "🏷", "Security": "🔒", "Spanning Tree": "🌳",
            "Resource": "⚡", "SD-WAN": "🛣", "General": "📋", "Unknown": "❓",
        }

        for log in filtered:
            sev  = log.get("severity", "info")
            ico  = "🔴" if sev == "critical" else "🟡" if sev == "warning" else "ℹ️"
            dot  = "tl-crit" if sev == "critical" else "tl-warn" if sev == "warning" else "tl-info"
            cat  = log.get("category", "General")
            cat_ico = category_icons.get(cat, "📋")
            ext  = log.get("extracted", {})

            # Build metric badges for IP SLA entries
            metric_badges = ""
            if log.get("type","").startswith("ipsla") or cat == "IP SLA":
                badge_parts = []
                if ext.get("rtt"):    badge_parts.append(f'<span class="nb-chip {"chip-err" if int(ext["rtt"])>150 else "chip-info"}">RTT {ext["rtt"]}ms</span>')
                if ext.get("jitter"): badge_parts.append(f'<span class="nb-chip {"chip-warn" if int(ext["jitter"])>30 else "chip-info"}">Jitter {ext["jitter"]}ms</span>')
                if ext.get("loss"):   badge_parts.append(f'<span class="nb-chip {"chip-err" if int(ext["loss"])>5 else "chip-info"}">Loss {ext["loss"]}%</span>')
                if ext.get("mos"):    badge_parts.append(f'<span class="nb-chip {"chip-err" if float(ext["mos"])<3.5 else "chip-info"}">MOS {ext["mos"]}</span>')
                if ext.get("probe_id"): badge_parts.append(f'<span class="nb-chip chip-info">Probe #{ext["probe_id"]}</span>')
                if ext.get("state"):  badge_parts.append(f'<span class="nb-chip {"chip-err" if ext["state"].lower()=="down" else "chip-ok"}">{ext["state"].upper()}</span>')
                metric_badges = " ".join(badge_parts)

            st.markdown(f"""<div class="nb-timeline-item">
              <div class="nb-tl-dot {dot}">{ico}</div>
              <div class="nb-tl-body" style="flex:1">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
                  <div style="flex:1">
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
                      <span style="font-size:11px">{cat_ico}</span>
                      <span class="nb-tl-title" style="font-family:JetBrains Mono,monospace;font-size:11px">{log.get('message','')[:110]}</span>
                    </div>
                    <div class="nb-tl-meta">{log.get('ts','')} · {log.get('device','')} · {cat}</div>
                    {f'<div style="margin-top:4px">{metric_badges}</div>' if metric_badges else ''}
                    <div class="nb-tl-ai">🧠 {log.get('ai_hint','')}</div>
                  </div>
                </div>
              </div>
            </div>""", unsafe_allow_html=True)

            if log.get("needs_action"):
                if st.button(f"🧠 AI Fix", key=f"log_fix_{log.get('ts','')}_{log.get('device','')[:8]}_{log.get('type','')[:8]}",
                             help=f"AI analysis for: {log.get('message','')[:50]}"):
                    go(
                        f"Fix this network log event:\n{log.get('raw', log.get('message',''))}\n\n"
                        f"Device: {log.get('device')} | Category: {cat} | Severity: {sev}\n"
                        f"Extracted data: {json.dumps(ext)}\n\n"
                        "Provide: root cause, exact CLI fix commands, verification steps, rollback.",
                        "troubleshoot",
                        f"Log analysis: {log.get('device')} {cat} {sev}",
                    )

        if st.button("🗑 Clear uploaded logs", key="syslog_clear"):
