...
if workspace == "copilot":
    ...
    try:
        ...
        _cp_approved_devs = _cp_disc.get_approved() or []
    except Exception as _e:
    ...
    _btn_col1, _btn_col2, _btn_col3, _spacer = st.columns([1, 1, 1, 2])
    ...
    with _btn_col1:
        ...
        if _uploaded_file:
            ...
            st.success(f"✅ Image uploaded: {_uploaded_file.name}")

    # Button 2: Network AI Mode
    with _btn_col2:
        st.markdown("**Network AI**")
        _ai_mode = st.selectbox(
            label="ai_mode_select",
            label_visibility="collapsed",
            options=["Net Config", "Net Fix", "Net Design"],
            key="copilot_ai_mode_select",
        )
        if _ai_mode:
            st.session_state["copilot_ai_mode"] = _ai_mode
            _mode_emojis = {
                "Net Config": "⚙️",
                "Net Fix": "🔧",
                "Net Design": "🎨",
            }
            st.success(f"{_mode_emojis.get(_ai_mode, '✨')} {_ai_mode} selected")

    # Button 3: Select Devices
    with _btn_col3:
        st.markdown("**Devices**")
        if _cp_approved_devs:
            # Create searchable device options
            _device_search = st.text_input(
                label="device_search",
                label_visibility="collapsed",
                placeholder="Search by name or IP...",
                key="copilot_device_search",
            )
            ...
            if _device_search.strip():
                ...
                _filtered_devices = [
                    d for d in _cp_approved_devs
                    if _search_lower in d.hostname.lower() or _search_lower in d.ip.lower()
                ]
            else:
                _filtered_devices = _cp_approved_devs
            
            # Display available devices count
            st.caption(f"Found: {len(_filtered_devices)} device(s)")
            
            # Multiselect from filtered devices
            _device_options = [f"{d.hostname or d.ip} ({d.ip})" for d in _filtered_devices]
            _selected_device_labels = st.multiselect(
                label="device_selector_multi",
                label_visibility="collapsed",
                options=_device_options,
                key="copilot_device_multiselect",
            )
            if _selected_device_labels:
                st.session_state["copilot_selected_devices"] = _selected_device_labels
                st.success(f"✅ {len(_selected_device_labels)} device(s) selected")
        else:
            st.warning("ℹ️ No approved devices yet")
    ...
    if st.session_state["copilot_chat_messages"]:
        ...
        for _msg in st.session_state["copilot_chat_messages"]:
            if _msg["role"] == "user":
                st.markdown(f"""
                <div style="background:#0e151f;border-left:4px solid #2563eb;border-radius:8px;
                            padding:12px 16px;margin-bottom:12px;font-size:14px">
                  <div style="color:#4c8dff;font-weight:600;margin-bottom:4px">👤 You</div>
                  <div style="color:#f0f4fa">{_msg['content']}</div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div style="background:#0e151f;border-left:4px solid #3fd27a;border-radius:8px;
                            padding:12px 16px;margin-bottom:12px;font-size:14px">
                  <div style="color:#3fd27a;font-weight:600;margin-bottom:4px">🤖 Copilot</div>
                  <div style="color:#c8d6e8;line-height:1.6">{_msg['content']}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with _clear_col2:
            if st.button("🗑 Clear Chat", key="cp_clear_chat", use_container_width=True):
                st.session_state["copilot_chat_messages"] = []
                st.session_state["copilot_selected_devices"] = []
                st.session_state["copilot_uploaded_image"] = None
                st.session_state["copilot_ai_mode"] = None
    ...
    if _input_text and _input_text.strip():
        ...
        if st.session_state["copilot_ai_mode"]:
            _context_parts.append(f"Mode: {st.session_state['copilot_ai_mode']}")
        
        # Add selected devices
        if st.session_state["copilot_selected_devices"]:
        ...
        if st.session_state["copilot_uploaded_image"]:
            _context_parts.append(f"[Image uploaded: {st.session_state['copilot_uploaded_image'].name}]")

        _context_str = " | ".join(_context_parts)
        _full_prompt = f"""You are Network Intelligence Copilot, an expert network operations assistant.

{_context_str}

User query: {_q}

Provide a comprehensive, technically accurate response. If the query involves configuration, include specific CLI commands. Be concise but thorough."""

        with st.spinner("✨ Network Copilot is thinking…"):
            try:
                _ai_reply = call_ai(_full_prompt)
            except Exception as _e:
                _ai_reply = f"❌ Error: {_e}"
