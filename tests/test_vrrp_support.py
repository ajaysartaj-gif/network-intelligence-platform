...
def _make_ai(query_state_word):
    ...
    def ai(prompt: str) -> str:
        ...
        if "Interpret this device output" in prompt:
            ...
            # compiled VRRP hypothesis is seeded regardless of scenario, and
            # Backup's statement always contains "preempt", so an ungated
            # search would wrongly ground it even while testing Initialize.
            if query_state_word == "Backup":
                m = re.search(r"\[(hyp_[0-9a-f]+)\][^\n]*preempt", prompt)
                if m:
                    impacts.append({"hypothesis_id": m.group(1), "effect": "support",
                                    "weight": 0.6,
                                    "reason": "show vrrp confirms Backup state, no preempt configured"})
            facts = [{"subject": "vrrp.neighbor", "attribute": "state", "value": "BACKUP"}]
