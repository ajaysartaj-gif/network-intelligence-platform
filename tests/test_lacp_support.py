...
def _make_ai(query_state_word):
    ...
    def ai(prompt: str) -> str:
        ...
        if "Interpret this device output" in prompt:
            ...
            # compiled LACP hypothesis is seeded regardless of scenario, and
            # Individual's statement always contains "LACPDUs", so an
            # ungated search would wrongly ground it even while testing
            # e.g. Suspended.
            if query_state_word == "Individual":
                m = re.search(r"\[(hyp_[0-9a-f]+)\][^\n]*LACPDUs", prompt)
                if m:
                    impacts.append({"hypothesis_id": m.group(1), "effect": "support",
                                    "weight": 0.6,
                                    "reason": "show etherchannel summary shows port stuck Individual (I flag)"})
            facts = [{"subject": "lacp.neighbor", "attribute": "state", "value": "INDIVIDUAL"}]
