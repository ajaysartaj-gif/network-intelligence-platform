"""
core/knowledge/fetchers/dell_fetcher.py
=========================================
Dell EMC Networking (OS10 / OS9, PowerSwitch) documentation fetcher — same
search-first pattern as fortinet_fetcher.py.
"""
...
class DellFetcher(VendorFetcher):
    """Dell EMC Networking OS10 / OS9 documentation fetcher."""

    vendor_key   = "dell"
    display_name = "Dell EMC Networking"
    trusted_domains = ["dell.com"]
    ...
    def search_candidates(
        ...
        queries = [
            f'"{command}" Dell OS10 "command line reference" site:dell.com',
            f'"{command}" Dell PowerSwitch site:dell.com',
        ]
        ...
        for q in queries:
            for url, title in self._duckduckgo_search(q):
                if url in seen or "dell.com" not in url:
                    continue
                seen.add(url)
    ...
    def parse_page(
        ...
        version_match = re.search(r"OS1?0?\s*\d+(?:\.\d+)*", soup.get_text())
